    ...
    with open('../data/clean_match.sql', 'r', encoding='utf-8') as f:
        ...
        for match_id in match_ids:
            for sql in lines:
                ...
                db_utils.get_lottery_db().modify_execute(sql_r)
...
def clean_match_order(match_id):
    ...
    order_id = db_utils.get_lottery_db().query_execute(sql_order_id)
    ...
    if order_id:
        ...
        db_utils.get_lottery_db().modify_execute(sql_delete_option_order)
        sql_delete_order = f"delete from sport_betting_order where id in {delete_id}"
        db_utils.get_lottery_db().modify_execute(sql_delete_order)
    else:
