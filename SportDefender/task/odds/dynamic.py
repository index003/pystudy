...
def initial_return_rate(odds_list):
    ...
    return std_out.format_float(r_rate)
...
def initial_win_rate(odds_list):
    ...
    w_rate = []
    for odds in odds_list:
        w_rate_item = (odds, std_out.format_float(r_rate / odds))
...
def get_k_values(odds_list, k):
    ...
    for w_rate in w_rates:
        k_value = (w_rate[0], std_out.format_float(w_rate[1] * k))
...
def get_target_odds_total_amount(odds, bet_amount, k, odds_list):
    ...
    return std_out.format_float(target_odds_total_amount)
...
def get_other_odds_total_amount(k, odds_list):
    ...
    for k_value in k_values:
        ...
        other_odds_total_amounts.append(std_out.format_float(other_odds_total_amount))
...
def get_dynamic_odds(odds, bet_amount, k, odds_list):
    ...
    aat = std_out.format_float(target_odds_total_amount / all_amount)
    aat_odds = std_out.format_float(r_rate / aat, 2)
    ...
    for other_odds_total_amount in other_odds_total_amounts:
        bbt = std_out.format_float(other_odds_total_amount / all_amount)
        bbt_odds = std_out.format_float(r_rate / bbt, 2)
